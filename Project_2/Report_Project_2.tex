...
As we can see, the numerical solutions are close to the analytical with deviation of order $\sim 10^{-4}$. The analytical solutions are gotten from the well-known energy formula for the harmonic oscillator, $E_n=\hbar\omega(2n+3/2)$.[\cite [p.47]{Griffiths}] In our case, we are multiplying with $2/\hbar\omega$. 
