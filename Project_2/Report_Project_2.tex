...
\begin{abstract}
This project aims to simulate so-called quantum dots, electrons trapped in harmonic oscillator potentials. By rewriting the Schroedinger equation to an eigenvalue problem which we solved with Jacobi's rotational method. 
...
As we can see, the numerical solutions are close to the analytical with deviation of order $\sim 10^{-4}$. The analytical solutions are gotten from the well-known energy formula for the harmonic oscillator, $E_n=\hbar\omega(n+1/2)$.[\cite [p.25]{Griffiths}] In our case, we are multiplying with $2/\hbar\omega$. 
...
<<<<<<< HEAD
Table 5 and 6 consist of different benchmarks, where the first is analytical solutions compared with numerical solutions of the one electron case and the latter is analytical solution compared with numerical solution of the two interacting electrons case. We saw that the deviations were small when we ran for $N=350$, something which indicates that our implementations are correct.
...
=======
Table 5 and 6 consists of different benchmarks, where the first is analytical solutions compared with numerical solutions of the one electron case and the latter is analytical solutions compared with numerical solutions of the two interacting electrons case. We saw that the deviations were small when we run for $N=350$, something which indicates that our implementations are correct.
>>>>>>> 307f54f186c398dace06a4c07772dfed12a534a7

The benchmarks when we 
We also have done some benchmarks where we are comparing our numerical results with analytical solutions. Many problems when we got interacting atoms have not analytical solutions, but actually our case with two interacting atoms has analytical energy solutions. 
