...
\end{equation}
where $G$ is the gravitational constant, $m_1$ and $m_2$ are the masses of the objects, $r$ is the distance between them and $\hat{r}$ is an unit vector in radial direction. Since we are using astronomical units, this gives the value $G = 4\pi^2 M_{\odot} ^{-1} AU^3 yr^{-2}$ \cite{Project_text} for the gravitational constant. The absolute gravitational force between two objects is therefore
\begin{equation}
...
The object oriented approach makes it so that in order to perform an experiment, we simply have to decide which planets and initial conditions we want, which ODE solver we want and 
\section{Results}
\section{Discussion}
\section{Conclusion}
\section{References}
