...
void RelativisticNewtonianGravity::computeForces(Particle& a, Particle& b) {
    ...
    double dFx = -m_G*(a.getMass()*b.getMass())/(drlength*drlength2)*b.getPosition()(0)*(1+ (3*(dr.cross(b.getVelocity())).lengthSquared())/(drlength2*pow(c,2)));
    double dFy = -m_G*(a.getMass()*b.getMass())/(drlength*drlength2)*b.getPosition()(1)*(1+ (3*(dr.cross(b.getVelocity())).lengthSquared())/(drlength2*pow(c,2)));
    double dFz = 0.0;
